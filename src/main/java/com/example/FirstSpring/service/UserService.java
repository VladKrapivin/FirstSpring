...
public class UserService implements UserDetailsService {
    ...
    public boolean deleteUser(Long userId) {
        if (userRepository.findById(userId).isPresent()) {
            ...
            List<UserManagerAC> userManagerACS = userManagerACRepository.findByUser(user);
            List<TemporaryUserManager> temporaryUserManagers = temporaryUserManagerRepository.findByUser(user);
            List<UserManagerAC> userManagerACs1 = userManagerACRepository.findByManager(user);
            for(UserAC AC : userACS) {
                userACRepository.delete(AC);
            }
            for(UserManagerAC AC : userManagerACS) {
                userManagerACRepository.delete(AC);
            }
            for(TemporaryUserManager AC : temporaryUserManagers) {
                temporaryUserManagerRepository.delete(AC);
            }
            for(UserManagerAC AC : userManagerACs1) {
                userManagerACRepository.delete(AC);
            }
